...
function levelToConfig(level: number): { skill: number; searchDepth: number } {
  // Level 1 = very easy, Level 10 = maximum
  const configs: Record<number, { skill: number; searchDepth: number }> = {
...
export function initEngine(_configDepth?: number) {
  // Depth is controlled per-level via levelToConfig
}

export function getBestMove(fen: string, level: number): Promise<string> {
